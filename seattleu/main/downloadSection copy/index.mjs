...
async function main(instance) {
  ...
  // for (const item of allContents) {
    ...
    allData.push({contentid: item.id, id : contentItem.id, contentID : contentItem.contentTypeID, status : item.status, name : contentItem.name, elements : contentItem.elements, types : contentItem.types, 
...
function createExcelFiles(data) {
  ...
  Object.entries(groupedBySection).forEach(([parentSectionId, records]) => {
    ...
    Object.entries(groupedByContentID).forEach(([contentID, entries]) => {
      // Updated headers to include an empty column, Section ID and Content ID
      const headers = ['', 'Section ID', 'Content ID', ...entries[0].headerInfo.map(header => header.name)];

      // Updated header types with empty values for the empty column, section ID and content ID columns
      const headerTypes = ['', '', '', ...entries[0].headerInfo.map(header => 
        entries[0].types.find(type => type.id === parseInt(header.type))?.name || header.type
      )];
      
      // First row is header types, second row is headers
      ...
      entries.forEach(entry => {
        const elementValues = entries[0].headerInfo.map(header => {
          ...
          if (cellData.length > 32767) {
            console.warn(`Data truncated for cell: ${header.name}, original length: ${cellData.length}`);
            cellData = cellData.substring(0, 32767);
          }
          return cellData;
        });
        
        // Add empty cell, section ID and content ID as the first three columns in each row
        const row = ['', entry.parentSectionId, entry.id, ...elementValues];
        ws_data.push(row);
      });

      // Add content type ID to the first cell only, don't offset other headers
      ws_data[0][0] = contentID;  
      
      const worksheet = XLSX.utils.aoa_to_sheet(ws_data);
      
      // Updated column widths for the new structure
      worksheet['!cols'] = [
        { wch: 5 },  // Empty column
        { wch: 10 }, // Section ID column
        { wch: 10 }, // Content ID column
        ...Array(ws_data[0].length - 3).fill({ wch: 20 })
      ];
...
async function getAllContents(section, hierarchy, currentDepth = 0, maxDepth = 3) {
  ...
  console.log(`Contents for section ${section.id} (depth ${currentDepth}):`, contentList);
